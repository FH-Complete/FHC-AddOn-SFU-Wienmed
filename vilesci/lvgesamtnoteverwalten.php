...
?>
	...
	function GradeImport()
	{
		...
		str += '<tr><td><input type="text" name="benotungsdatum" id="benotungsdatum" value="<?php echo date('Y-m-d') ?>"> (Benotungsdatum)</td></tr>';
	...
	function saveGradeBulk()
	{
		...
		benotungsdatum = $('#benotungsdatum').val();
		closeDiv();
		...
		if (i>0)
		{
			...
			var ts = jetzt.getTime();
			var url= '<?php echo "lvgesamtnoteeintragen.php?lvid=".urlencode($lvid)."&stsem=".urlencode($stsem); ?>';
			url += '&submit=1&'+ts+'&benotungsdatum='+benotungsdatum;
			$.ajax({
				type:"POST",
				url: url,
				data: gradedata,
...
function setNewLV()
{
	newid = document.getElementById('set_lvid').value;
	//alert(newid);
	semester = '<?php echo $stsem; ?>';
	window.location.href='lvgesamtnoteverwalten.php?lvid='+newid+'&stsem='+semester;
...
// Eingabefeld für LVID:

echo 'LV-ID: <input type="text" name="set_lvid" id="set_lvid" value="'.$lvid.'"><input type="button" name="setlv" value="Set" onclick="setNewLV()">&nbsp;';
