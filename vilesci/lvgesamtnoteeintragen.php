...
function savenote($db,$lvid, $student_uid, $note, $punkte=null, $benotungsdatum=null)
{
    ...
    if (!$lvgesamtnote->load($lvid, $student_uid, $stsem))
    {
		...
		$lvgesamtnote->benotungsdatum = $benotungsdatum;
...
{
	$lvid = $_REQUEST["lvid"];
	if (isset($_REQUEST["benotungsdatum"]))
		$benotungsdatum = $_REQUEST["benotungsdatum"];
	else
		$benotungsdatum = null;
		...
		foreach ($_POST as $row=>$val)
		{
			if(mb_strstr(mb_strtolower($row), 'matrikelnr_'))
			{
				...
				if(isset($_POST['matrikelnr_'.$id]) && (isset($_POST['note_'.$id]) || isset($_POST['punkte_'.$id])))
				{
					...
					//UID ermitteln
					$student = new student();
					if ($student->load($matrikelnummer))
						$student_uid = $matrikelnummer;
					else if(!$student_uid = $student->getUidFromMatrikelnummer($matrikelnummer))
					{
					...
					$val=savenote($db,$lvid, $student_uid, $note, $punkte, $benotungsdatum);
